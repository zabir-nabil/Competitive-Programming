...
int lim[45];
ll dp[45][4][4][4];
ll rec(int pos,int lbit,int cbit,int overf)
{
    if(pos==4)
        return 0;

    if(dp[pos][lbit][cbit][overf]!=-1)
        return dp[pos][lbit][cbit][overf];
    ...
    if(ov)
    {
        p1 += rec(pos+1,cbit,0,ov);

        if(lim[pos])
        {
            p1 += rec(pos+1,cbit,1,ov);
